...
namespace xtozero
{
	...
	public:
		...
		Edge( int minY, int maxY, float minX, float maxX, float startZ, float endZ, float startW, float endW, float gradient,
			const Vector3& startLocalVertex, const Vector3& endLocalVertex )
			: m_minY( minY ), m_maxY( maxY ), m_minX( minX ), m_maxX( maxX ), 
			m_startZ( startZ ), m_endZ( endZ ), m_startW(startW), m_endW(endW), m_gradient( gradient ),
	...
	private:
		...
		std::vector<std::tuple<int, float, float, Vector3>> m_horizontalLine;
	...
	public:
		...
		void ProcessScanlineParallel( int scanline, unsigned int facecolor, std::vector<Edge>& activeEdgeTable, 
			std::vector<CPsElementDesc>& outputRS, std::vector<std::tuple<int, float, float, Vector3>>& horizontalLine );
