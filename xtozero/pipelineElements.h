...
namespace xtozero
{
	...
	public:
		...
		CPsElementDesc( const CPsElementDesc& element ) 
			: m_x( element.m_x ), m_y( element.m_y ), m_z( element.m_z ), m_invZ( element.m_invZ ), m_color( element.m_color ) {}
		CPsElementDesc( const int x, const int y, const float z, const float invZ, const unsigned int color, const Vector3& localVertex )
			: m_x( x ), m_y( y ), m_z( z ), m_invZ(invZ), m_color( color ), m_localVertex( localVertex ) {}
		~CPsElementDesc( ) {}
