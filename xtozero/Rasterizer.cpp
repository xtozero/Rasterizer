...
namespace xtozero
{
	...
	void CRasterizer::ProcessScanline( int scanline, unsigned int facecolor )//정점 보간하면 컬러 넘겨주지 않을 예정...
	{
		...
		for ( std::vector<Edge>::iterator iter = m_activeEdgeTable.begin(); iter != m_activeEdgeTable.end(); ++iter )
		{
			...
			else
			{
				...
				float invZ = Lerp( 1.0f / edge.m_startW, 1.0f / edge.m_endW, lerpRatio );
				Vector3 localVertex = Lerp( edge.m_startLocalVertex, edge.m_endLocalVertex, lerpRatio );

				m_horizontalLine.emplace_back( static_cast<int>(intersectX), z, invZ, localVertex );
			}
		}
		...
		Vector3 startLocalVertex;
		Vector3 endLocalVertex;
		for ( std::vector<std::tuple<int, float, float, Vector3>>::iterator iter = m_horizontalLine.begin(); iter != m_horizontalLine.end(); iter += 2 )
		{
			std::tuple<int, float, float, Vector3>& posXZ_LocalVertex = *iter;
			...
			if ( (iter + 1) == m_horizontalLine.end() )
			{
				...
				m_outputRS.emplace_back( x, scanline, std::get<1>( posXZ_LocalVertex ), std::get<2>( posXZ_LocalVertex ), facecolor, std::get<3>( posXZ_LocalVertex ) );
				break;
			}

			std::tuple<int, float, float, Vector3>& posNextXZ_LocalVertex = *(iter + 1);
			...
			startInvZ = std::get<2>( posXZ_LocalVertex );
			endInvZ = std::get<2>( posNextXZ_LocalVertex );
			startLocalVertex = std::get<3>( posXZ_LocalVertex );
			endLocalVertex = std::get<3>( posNextXZ_LocalVertex );
			if ( startX > endX )
			{
				...
				startInvZ = std::get<2>( posNextXZ_LocalVertex );
				endInvZ = std::get<2>( posXZ_LocalVertex );
				startLocalVertex = std::get<3>( posNextXZ_LocalVertex );
				endLocalVertex = std::get<3>( posXZ_LocalVertex );
			}
			...
			float invZGradient = (endInvZ - startInvZ) / (endX - startX);
			Vector3 localVertexGradient = (endLocalVertex - startLocalVertex) / (static_cast<float>(endX - startX));
			for ( int i = startX; i <= endX; ++i )
			{
				m_outputRS.emplace_back( i, scanline, z, invZ, facecolor, localVertex );
				z += zGradient;
				invZ += invZGradient;
	...
	void CRasterizer::ProcessScanlineParallel( int scanline, unsigned int facecolor, std::vector<Edge>& activeEdgeTable,
		std::vector<CPsElementDesc>& outputRS, std::vector < std::tuple < int, float, float, Vector3 >> &horizontalLine )//정점 보간하면 컬러 넘겨주지 않을 예정...
	{
		...
		for ( unsigned int i = 0; i < activeEdgeTable.size(); ++i )
		{
			...
			else
			{
				...
				horizontalLine.emplace_back( static_cast<int>(intersectX), z, invZ, localVertex );
			}
		}
		...
		{
			std::tuple<int, float, float, Vector3>& posXZ_LocalVertex = horizontalLine[i];
			...
			if ( (i + 1) == horizontalLine.size( ) )
			{
				...
				outputRS.emplace_back( x, scanline, std::get<1>( posXZ_LocalVertex ), std::get<2>( posXZ_LocalVertex ), facecolor, std::get<3>( posXZ_LocalVertex ) );

				break;
			}

			std::tuple<int, float, float, Vector3>& posNextXZ_LocalVertex = horizontalLine[i + 1];
			...
			for ( int i = startX; i <= endX; ++i )
			{
				outputRS.emplace_back( i, scanline, z, invZ, facecolor, localVertex );
				z += zGradient;
				invZ += invZGradient;
				localVertex += localVertexGradient;
			}
				...
				for ( int i = minY; i <= maxY; ++i )
				{
					...
					for ( int j = minX; j <= maxX; ++j )
					{
						if ( tempS0 >= 0 && tempS1 >= 0 && tempS2 >= 0 )
						{
							...
							m_outputRS.emplace_back( j, i, z, invZ, facecolor, localVertex );
						}
	...
	void RsThreadWork( LPVOID arg )
	{
		...
		std::vector<std::tuple<int, float, float, Vector3>> horizontalLine;
